...
void citire_graf(int nr_muchi , vector<pair<int,int> > *la,int *control,int& cate)
{
	int a,b,c;
	a=b=c=0;
	...
	scanf("%d",&cate);
	for(int i=0;i<cate;i++)scanf("%d",&control[i]);
}
...
void algoritm(vector<pair<int,int> > *la,int n,int m,int start,int* control,int cate)
{
	//variabile
	...
	for(int i=1;i<=n;i++)
	{
		...
		for(int k=0;k<cate;k++)
            if(control[k]==curent)
            {
                printf("Punct de control cel mai apropiat: %d\n",curent);
                while(curent)
                {
                    printf("%d ",curent);
                    curent=tata[curent];
...
int main()
{
	...
	control = new int[n+1];
	citire_graf(m,la,control,cate);

    start=3;
	algoritm(la,n,m,start,control,cate);
	return 0;
}
